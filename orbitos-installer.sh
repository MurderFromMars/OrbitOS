...
# ── Non-fatal systemd unit enable ────────────────────────────────────────────
# A missing unit makes `systemctl enable` exit 1, which under `set -e` kills
# the whole installer. Enable units one at a time and warn instead of dying.
_enable_units() {
    local u
    for u in "$@"; do
        arch-chroot "$ORBIT_MOUNT" systemctl enable "$u" 2>/dev/null \
            && ui_ok "enabled $u" \
            || ui_warn "unit not found, skipped: $u"
    done
}
...
EOF
    ...
    _enable_units NetworkManager.service avahi-daemon.service
...
setup_handheld_services() {
    ...
    _enable_units "hhd@${CFG[username]}"
}
...
install_kde_minimal() {
    ...
    # ── Power management: power-profiles-daemon only ──────────────────────
    # tuned-ppd conflicts with power-profiles-daemon; if an earlier
    # transaction (e.g. cachyos-settings) pulled it in, drop it first so
    # the --noconfirm install below doesn't abort on the conflict.
    arch-chroot "$ORBIT_MOUNT" pacman -Rdd --noconfirm tuned-ppd 2>/dev/null || true
    ...
    case "${CFG[login_manager]}" in
        plasma-login)
            ...
            _enable_units plasmalogin.service
            ;;
        *)
            arch-chroot "$ORBIT_MOUNT" pacman -S --noconfirm --needed sddm
            _enable_units sddm.service
            ;;
    esac
    ...
    # NOTE: wpa_supplicant.service is intentionally NOT enabled here —
    # NetworkManager D-Bus-activates it on demand, and a standalone
    # enabled instance can race the NM-managed one.
    _enable_units \
        cups.socket \
        bluetooth.service \
